...
def load_client(client_id):
    return wbankclients.query.filter_by(client_id=client_id).first()

@oauth.grantgetter
def load_grant(client_id, code):
    return wbankgrants.query.filter_by(client_id=client_id, code=code).first()
...
def load_token(access_token=None, refresh_token=None):
    if access_token:
        return wbankapitoken.query.filter_by(access_token=access_token).first()
    elif refresh_token:
        return wbankapitoken.query.filter_by(refresh_token=refresh_token).first()

@oauth.tokensetter
def save_token(token, request, *args, **kwargs):
    toks = wbankapitoken.query.filter_by(client_id=request.client.client_id,
                                 user_id=request.user.id)
    # make sure that every client has only one token connected to a user
    for t in toks:
        db.session.delete(t)

    expires_in = wbankapitoken.get('expires_in')
