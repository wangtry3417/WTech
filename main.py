...
def wbank_hash_transfer():
  ...
  if count >= 5000000.00 and users.sub != "此為WBank帳戶,Not access to login":
...
def wbank_paypal_done():
  user = request.args.get("user")
  count = float(request.args.get("amount"))/3.5
...
def wbank_card_hash_action():
  if request.method == "GET":
    ...
    users = wbankwallet.query.all()
    for user in users:
      if data["password"].strip() != user.password:
        continue
      cardno = f"{user.accnumber}->{user.password}"
      hash_code = hashlib.sha256(cardno.encode()).hexdigest()
